...
-- ************************************************************************************************************************************************
-- ******************************************** Create new schedule for Index Reorganise job ******************************************************
-- ************************************************************************************************************************************************
...
IF @ip LIKE '10.4.%' 
       ...
       IF (SELECT COUNT(Job_id) FROM msdb.dbo.sysjobs WHERE [name] LIKE 'DBMP%index reo%' /*OR [name] LIKE 'DBMP%Backup full%'*/) = 1
              BEGIN
              SELECT @Daily_job_id = [job_id] FROM msdb.dbo.sysjobs WHERE [name] LIKE 'DBMP%index reo%' -- OR [name] LIKE 'DBMP%Backup full%'
              SELECT @Daily_job_name = [name] FROM msdb.dbo.sysjobs WHERE [name] LIKE 'DBMP%index reo%' -- OR [name] LIKE 'DBMP%Backup full%'
              SET @name_for_new_schedule = @Daily_job_name + ' (Wed. midday)'
              ...
              IF NOT EXISTS (SELECT [schedule_id] FROM msdb.dbo.sysschedules WHERE [NAME] = @name_for_new_schedule) 
                     ...
                     EXEC msdb.dbo.sp_attach_schedule @job_id=@Daily_job_id, 
                                                                                           @schedule_name=@name_for_new_schedule                
                     END
              ELSE
                     BEGIN
       ...
       ELSE
              BEGIN
              PRINT 'SQL agent job using the naming standard DBMP%index reo% not found...'
              END
       END    
ELSE
       BEGIN
