...
SET /p DBAPASSWORD="Please provide password for SQL login 'DBA' (available in KeePass): "
...
sqlcmd -S %SERVER% -d DBA -v var_environment=%ENVIRONMENT% var_shared=%SHARED% -i Phase99_Step98_PopulateTables_Parameters_table.sql -o Phase99_Step98_PopulateTables_Parameters_table.sql.log
timeout /t 10 /nobreak > NUL
Notepad.exe Phase99_Step98_PopulateTables_Parameters_table.sql.log

REM **** Create linked server in DBA Central ****
SET DBACENTRALSERVER=DBA_Central.db.sth.health.qld.gov.au,21433
sqlcmd -S "%DBACENTRALSERVER%" -d DBA -v var_server="%SERVER%" var_password="%DBAPASSWORD%" -i Phase99_Step99_Create_linked_server_on_DBA_central_server.sql -o Phase99_Step99_Create_linked_server_on_DBA_central_server.sql.log
timeout /t 10 /nobreak > NUL
Notepad.exe Phase99_Step99_Create_linked_server_on_DBA_central_server.sql.log
