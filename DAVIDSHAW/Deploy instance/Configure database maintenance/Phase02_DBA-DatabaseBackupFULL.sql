...
/****** Object:  Job [DBA - Database backup (FULL)]    Script Date: 31/05/2021 11:36:38 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 31/05/2021 11:36:38 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - Database backup (FULL)', 
		@enabled=1, 
...
1) Delete previous full backups that has passed the rentention designated period.
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Delete old FULL backups]    Script Date: 31/05/2021 11:36:38 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Delete old FULL backups', 
		@step_id=1, 
...
EXECUTE master.dbo.xp_delete_file 0,@DefaultDirectory,N''bak'',@today_minus_1_day,1', 
		@database_name=N'master', 
		@output_file_name=@path_string, 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DatabaseBackup (FULL)]    Script Date: 31/05/2021 11:36:38 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'DatabaseBackup (FULL)', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [sp_DatabasesCollector]    Script Date: 31/05/2021 11:36:38 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'sp_DatabasesCollector', 
		@step_id=3, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Daily - 7:00 pm', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
