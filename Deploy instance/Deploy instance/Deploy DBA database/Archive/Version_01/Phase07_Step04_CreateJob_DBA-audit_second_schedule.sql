...
-- ************************************************************************************************************************************************
-- ******************************************** Create new schedule for Index Rebuild job *********************************************************
-- ************************************************************************************************************************************************
...
IF @ip LIKE '10.4.%' 
       ...
       IF (SELECT COUNT(Job_id) FROM msdb.dbo.sysjobs WHERE [name] LIKE 'DBMP%index reb%' /*OR [name] LIKE 'DBMP%Backup full%'*/) = 1
              BEGIN
              SELECT @Daily_job_id = [job_id] FROM msdb.dbo.sysjobs WHERE [name] LIKE 'DBMP%index reb%' -- OR [name] LIKE 'DBMP%Backup full%'
              SELECT @Daily_job_name = [name] FROM msdb.dbo.sysjobs WHERE [name] LIKE 'DBMP%index reb%' -- OR [name] LIKE 'DBMP%Backup full%'
              SET @name_for_new_schedule = @Daily_job_name + ' (Mon. midday)'
