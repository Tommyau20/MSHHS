...
--SET @job_owner = 'PA-HEALTH\sqlmaintenanceplan' 
SET @operator = 'CI_OPS' 

DECLARE job_cursor CURSOR FOR  
	--SELECT job_id FROM msdb..sysjobs ORDER BY [name]
	SELECT job_id FROM msdb..sysjobs WHERE [name] LIKE 'DBMP_%' OR [name] = 'syspolicy_purge_history'  ORDER BY [name] 
