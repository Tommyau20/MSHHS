...
/****** Object:  Job [DBA - Database integrity check]    Script Date: 31/05/2021 11:38:03 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 31/05/2021 11:38:03 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - Database integrity check', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DatabaseIntegrityCheck (ALL_DATABASES)]    Script Date: 31/05/2021 11:38:03 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'DatabaseIntegrityCheck (ALL_DATABASES)', 
	...
	BEGIN -- ***** execute on AlwaysOn AG secondary nodes only, only checking databases NOT in an AG. *****
	EXECUTE [DBA].[dbo].[DatabaseIntegrityCheck] @Databases = ''ALL_DATABASES, -AVAILABILITY_GROUP_DATABASES'',
					              @LogToTable = ''Y''

	END
ELSE
	BEGIN -- ***** execute on any database, including those databases in on AlwaysOn AG on a primary nodes. *****
	EXECUTE [DBA].[dbo].[DatabaseIntegrityCheck] @Databases = ''ALL_DATABASES'',
					              @LogToTable = ''Y''
	END', 
		@database_name=N'DBA', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Weekdays (not Wed, Sat) - 10:00 pm', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=55, 
		@freq_subday_type=1, 
